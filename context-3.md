...
| Ekran | Açıklama |
|-------|----------|
| K1 | Dünya haritası — `dunya.png` fullscreen + logo |
| K2 | Ülke seçim grid — 10 ülke PNG kartı (84×84px) |
| K3 | Şehir listesi — seçili ülkenin şehirleri |
| K4 | Şehir haritası — poligon + bina ikonları |

K3→K4 geçişinde ülke GIF'i overlay olarak gösterilir (`expo-image`).
...
- **Vibrasyon:** `expo-haptics` — `Haptics.impactAsync(ImpactFeedbackStyle.Light)`
- **Tüm text:** `textTransform: 'uppercase'`
- **Geri butonu:** `◀` unicode — PNG kullanılmaz
- **GIF overlay:** `useState` ile `visible` kontrolü
- **K4 harita:** `react-native-svg` ile poligon çizimi — D3 kullanılmaz, koordinatlar `gamedata.ts`'de
...
│       └── K4_CityMap.tsx
├── constants/
│   ├── gamedata.ts       ← ülkeler, şehirler, binalar, koordinatlar
│   ├── colors.ts
│   └── assets.ts         ← BASE_URL + tüm asset URL sabitleri
└── store/
...
## PAKETLER
```bash
npx expo install expo-image expo-haptics expo-font
npx expo install react-native-svg
npx expo install @expo-google-fonts/oswald @expo-google-fonts/bebas-neue
